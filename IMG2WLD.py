...
class TerrariaWorld:
    ...
    def saveV2(self, save_file_path):
...
input_file_path = input("이미지 파일 콘솔에 드래그 앤 드롭하고 엔터 : ")

image = cv2.imread(input_file_path, cv2.IMREAD_COLOR_RGB)
print(type(image))
MAX_WIDTH = 6000
MAX_HEIGHT = 3000
while image.shape[0] > MAX_HEIGHT or image.shape[1] > MAX_WIDTH:
    image = cv2.resize(image, (image.shape[0]//2, image.shape[1]//2))

LAB_image_np = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2LAB)
...
output_file_path = input_file_path[:-4] + ".wld"

world.saveV2(output_file_path)